...
public class CAtest {
	...
	public static void main(String[] args) {
		SpringApplication.run(CAtest.class, args);
	}

@Bean
CommandLineRunner CommandLineRun (CartItemRepository cartItemRepo, CartRepository cartRepo, ProductRepository prodRepo, CategoryRepository catRepo, UserRepository userRepo, CartService cartService, PaymentTypeRepository payRepo) {
	return args ->{
		...
		cartService.removeProductFromCart(simonCart.getId(), book2.getId());
	        ...
	        catRepo.save(programming);
	        catRepo.save(entertainment);
	        catRepo.save(travel);
	        catRepo.save(business);
